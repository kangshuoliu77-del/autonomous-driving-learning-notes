...
## 🚀 升级：CasADi + IPOPT + 4角点碰撞检测

Phase 4 SLSQP MPC 成功率 60%，有两个根本缺陷：中心点碰撞盲区 + 优化器初值敏感。将优化器换为 CasADi + IPOPT，碰撞检测改为4角点后，成功率提升至 **100%（10/10）**。

| 维度 | SLSQP 版 | CasADi+IPOPT 版 |
|------|---------|----------------|
| 优化器 | scipy SLSQP（局部，数值梯度） | CasADi + IPOPT（内点法，自动微分） |
| 碰撞检测 | 中心点 | 4 角点（线性化近似） |
...
|---|---|
| Trial 1：θ=273°，大弧度转向 | Trial 4：θ=116°，70步最快 |

| ![](./result_casadi_success_3.png) | ![](./result_casadi_success_4.png) |
|---|---|
| Trial 8：θ=13°，正向起步最难 | Trial 9：θ=329°，远端起点 |

详细实验过程见 [EXPERIMENT_LOG.md](./EXPERIMENT_LOG.md)。
