...
class KinematicCar:
    ...
    def get_corners(self, state):
        ...
        # --- 3. 定义身体系下的静态顶点 ---
        # 以“后轴中心”为原点 (0,0)
        f_x = self.length - self.rear_to_axle # 前边界坐标
        b_x = -self.rear_to_axle              # 后边界坐标
        l_y = self.width / 2.0                # 左边界坐标
        r_y = -self.width / 2.0               # 右边界坐标

        # 顶点顺序：[前左, 前右, 后右, 后左]
        corners_b = [(f_x, l_y), (f_x, r_y), (b_x, r_y), (b_x, l_y)]

        # --- 4. SE(2) 齐次变换展开 ---
        # Ps = Ts_b * Pb 矩阵乘法的展开形式
        cos_t = math.cos(theta)
        sin_t = math.sin(theta)
        
        corners_s = []
        for (bx, by) in corners_b:
            # 旋转矩阵 R * [bx, by] + 平移向量 [x, y]
